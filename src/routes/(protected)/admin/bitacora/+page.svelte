<script lang="ts">
	...
	async function load(targetPage?: number, forceFetch = false) {
		...
		const cacheKey = `auditlogs_${limit}_${offset}_${filterMethod}_${querySearch}`;

		// 1. SWR Cache check
		const cached = getFromCache<AuditLogRead[]>(cacheKey);
...
<div class="admin-page fade-in">
	<div class="admin-header-container">
		...
		<div class="admin-toolbar gap-3">
			<div class="admin-search-wrapper w-full sm:w-64">
				...
				>
				<input type="text" placeholder="Buscar en bitácora..." bind:value={searchQuery} oninput={handleSearchInput} />
			</div>
			...
			<div class="admin-filters w-full justify-between lg:w-auto lg:justify-end">
				...
				<button type="button" class="admin-btn-secondary !p-2" onclick={async () => { await load(page, true); toast.info('Bitácora actualizada'); }} title="Actualizar">
	...
	{:else}
		<section class="admin-section">
			<div class="admin-table-wrapper" bind:this={tableWrapper} style="max-height: 65vh; overflow-y: auto; position: relative;">
