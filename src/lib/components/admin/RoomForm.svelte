                            ...
                            {#if room.images && room.images.length > 0}
                                {#each room.images as imgUrl, i}
                                    <div class="group relative aspect-square rounded-[20px] overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 transition-all hover:shadow-xl hover:scale-[1.02] {room.cover_image_url === imgUrl ? 'border-amber-500 shadow-lg shadow-amber-500/10' : 'border-slate-200 dark:border-slate-800'}">
                                        ...
                                        <!-- Overlay de control con distribución premium y espacial -->
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 pointer-events-none"></div>
                                        
                                        <!-- Botones Flotantes en Esquinas Superiores -->
                                        <div class="absolute top-2 left-2 right-2 flex justify-between items-center z-20">
                                            {#if room.cover_image_url !== imgUrl}
                                                <button type="button" class="bg-black/60 backdrop-blur-md text-white p-2 rounded-xl border border-white/10 hover:bg-amber-500 hover:border-amber-400 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-105 active:scale-95 shadow-md" title="Marcar como Portada" onclick={() => setAsCover(imgUrl)}>
                                            ...
                                            {:else}
                                                <!-- Espacio vacío si ya es portada (pues el badge de portada ya está ahí arriba) -->
                                                <div></div>
                                            {/if}
                                            
                                            <button type="button" class="bg-black/60 backdrop-blur-md text-white p-2 rounded-xl border border-white/10 hover:bg-red-600 hover:border-red-500 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-105 active:scale-95 shadow-md" title="Eliminar Imagen" onclick={() => removeImage(i)}>
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                            </button>
                                        </div>

                                        <!-- Botones de Movimiento en Parte Inferior -->
                                        <div class="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                            {#if i > 0}
                                                <button type="button" class="bg-black/60 backdrop-blur-md text-white p-2 rounded-xl border border-white/10 hover:bg-indigo-600 hover:border-indigo-500 hover:scale-105 active:scale-95 shadow-md transition-all" title="Mover a la izquierda" onclick={() => moveImage(i, 'left')}>
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                                </button>
                                            {/if}
                                            {#if i < room.images.length - 1}
                                                <button type="button" class="bg-black/60 backdrop-blur-md text-white p-2 rounded-xl border border-white/10 hover:bg-indigo-600 hover:border-indigo-500 hover:scale-105 active:scale-95 shadow-md transition-all" title="Mover a la derecha" onclick={() => moveImage(i, 'right')}>
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                                </button>
                                            {/if}
                                        </div>
