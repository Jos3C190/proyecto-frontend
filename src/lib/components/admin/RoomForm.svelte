                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[500px] overflow-y-auto pr-1">
                            {#if room.images && room.images.length > 0}
                                {#each room.images as imgUrl, i}
                                    <div class="group relative aspect-square rounded-[20px] overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:shadow-xl hover:scale-[1.02]">
                                        ...
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4">
                                            <button type="button" class="bg-red-500/90 backdrop-blur-md text-white p-2 rounded-xl border border-white/20 hover:bg-red-600 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300" onclick={() => removeImage(i)}>
